# -*- coding: utf-8 -*-
"""
Created on Mon May  5 16:19:43 2025
...
# ------------------- AVERAGE VALUE PER PIXEL GRID CELL -------------------
# Spatial join: assign each circle to a pixel grid polygon
joined = gpd.sjoin(gdf_circles, pixel_grid, how="inner", predicate="intersects")

# Group by pixel grid geometry (using index of pixel_grid)
avg_values = joined.groupby("index_right")["value_left"].mean().reset_index()
avg_values.columns = ["index_right", "avg_value"]

# Merge the averages back into the pixel grid
pixel_grid_avg = pixel_grid.reset_index().merge(avg_values, left_index=True, right_on="index_right")

# Drop NaNs (optional, if you want to ignore pixels with no overlapping circles)
pixel_grid_avg = pixel_grid_avg.dropna(subset=["avg_value"])
...
# ------------------- PLOT 1: Average Radiometer Reflectance -------------------
fig1, ax1 = plt.subplots(figsize=(10, 8))
pixel_grid_avg.plot(column='avg_value', cmap=cmap, norm=norm, edgecolor='red', linewidth=3, alpha=0.8, ax=ax1)
...
cbar1.set_label("Avg Reflectance (%)", fontsize=30)
